...
window.onload = function() {
  ...
  let sounds = {
    ...
    night: document.getElementById('night-sound'),
    snow: document.getElementById('snow-sound'),
    sunset: document.getElementById('sunset-sound'),
    moon: document.getElementById('moon-sound'),
    storm: document.getElementById('storm-sound')
  };
  ...
  powerButton.onclick = function() {
    let outerRim = document.querySelector('.outer-rim');
    outerRim.classList.toggle('power-on');

    if (outerRim.classList.contains('power-on')) {
      // Power is on
      if (currentSound) {
        currentSound.play();
      }
  ...
  document.querySelector('#rot-icons').addEventListener('click', () => {
    ...
    if (!document.querySelector('.outer-rim').classList.contains('power-on')) {
      return; // If power is off, do nothing
    }
    ...
    if (index == 0) {
      ...
      currentSound = sounds.snow;
    }

    if (currentSound && document.querySelector('.outer-rim').classList.contains('power-on')) {
      currentSound.play();
    }
